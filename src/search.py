...
def astar_search(problem, h=None):
    ...
    initial_f = initial_f = node.path_cost + h(node.state)

    frontier = PriorityQueue()
    frontier.put((initial_f, node))
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
                child_f = child.path_cost + h(child.state)
                frontier.put((child_f, child))

            elif child in frontier:
                existing_f = frontier[child]
                child_f = child.path_cost + h(child.state)

                if child_f < existing_f:
                    frontier.update_elem(child, (child_f, child))
